...
Usage:
python3 format_data.py [input_file] [output_file]
"""
...
def format_data(input_file, output_file):
    ...
    rows = [["iteration", "changed_precincts"]]
    rows[0] += [f"community_{i}" for i in range(len(community_stages[0]))]
    for i in iterations:
        row = []
        row.append(str(i))
        row.append(str(changed_precincts[i]))
        for c in range(len(community_stages[0])):
            print(gerrymandering_scores[i][0])
            row.append(
