...
def modify_for_population(communities_list, precinct_corridors, total_population, threshold, iterations):
    ...
    while success != "yes!":
        ...
        for community5 in communities_list:
            ...
            elif abs(community5.population - ideal_population)/ideal_population > list(worst_population.keys())[0]:
                del worst_population[list(worst_population.keys())[0]]
                worst_population[abs(community5.population - ideal_population)/ideal_population] = community5
        worst_population_community = list(worst_population.values())[0] 
        worst_population_id = worst_population_community.id
        if abs(worst_population_community.population - ideal_population)/ideal_population < threshold:
            success = 'yes!'
            break  
        ...
        border_communities = {(community.population - ideal_population)/ideal_population : community for community in communities_list if not community == worst_population_community 
                              if get_if_bordering(community, worst_population_community) == True}
        if len(border_communities) == 0:
            raise Exception('no communities bordering?')
        while (worst_population_community.population - ideal_population)/ideal_population > threshold:
            print(worst_population_id, abs(worst_population_community.population - ideal_population)/ideal_population, threshold)
            ...
            # find precincts bordering the best_community that are in the worst_pop community
        ...
        for num, precinct3 in enumerate(border_precincts[worst_population_community.id]):
            ...
            other_community_population_stat = other_community.population
        ...
        while worst_population_community.standard_deviation > threshold:
            ...
            print('other community, ', other_community.id, 'highest stdev community, ', worst_population_id)
            ...
            if high_precinct in worst_population_community.precincts.values():
                ...
                if isinstance(clip([other_community.coords, high_precinct.coords], 1), MultiPolygon):
                    ...
                    continue
                try:
                    worst_population_community.give_precinct(other_community, high_precinct.vote_id)
            ...
            # precinct is not in biggest stdev community
                ...
                if isinstance(clip([worst_population_community.coords, high_precinct.coords], 1), MultiPolygon):
                    ...
                    continue
                try:
                    other_community.give_precinct(worst_population_community, high_precinct.vote_id)
                except CreatesMultiPolygonException:
                    del precinct_exchanges_dict[highest_precinct_exchange]
                    continue
                # update number of precincts changed on this iteration
            ...
            community_change_snapshot = [other_community, worst_population_community]
            ...
            precinct_count += 1
            changing_stdev.append(str(average([worst_population_community.standard_deviation, other_community.standard_deviation])) + '\n')
        ...
        with open('../../../../population_precinct_stdevs.txt', 'a') as f:
    ...
    communities_to_json(minimized_communities, '../../../../population_after.json')
    print('completed!')
    return (minimized_communities, precinct_corridors), count, standard_deviations, num_of_changed_precincts, population_stdev
...
b, count1, standard_deviations1, num_of_changed_precincts1, average_stdev1 = modify_for_population(x[0], x[1], 5, 50)
